...
static struct channel_context_st channel[MAXCHANNELID+1];//0下标空出来
...
static struct channel_context_st *path2entry(const char* path)
{
    ...
    static uint8_t curr_id = MINCHANNELID;
    ...
    {
        perror("fget()");
        //syslog(LOG_INFO, "%s is not a channel dir(cant get the .desc)", path);
        fclose(fp);
        return NULL;
    }
    ...
    me->desc = malloc(LINEBUFSIZE);
    memcpy(me->desc,linebuf,LINEBUFSIZE);
    ...
    if(glob(pathstr, 0, NULL, &me->mp3glob) != 0)
    {
        ...
        fprintf(stderr, "%s is not a channel dir(can not find mp3 files", path);
        free(me);
        return NULL;
    }
    me->pos = 0;
    ...
    if(me->fd <0 )
    {
        ...
        fprintf(stderr, "%s open failed.",me->mp3glob.gl_pathv[me->pos]);
        free(me);
        return NULL;
    }
    me->chnid = curr_id;
...
int mlib_getchnlist(struct mlib_listentry_st **result, int *resnum)
{
    ...
    char path[PATHSIZE];
    glob_t globres;
    struct mlib_listentry_st *ptr;//数组
    struct channel_context_st *res;

    for(int i = 0 ; i < MAXCHANNELID; i++)
    {
        channel[i].chnid = -1;//初始化 -1表示未启用
    }
    ...
    for(i = 0; i < globres.gl_pathc; i++)
    {
        ...
        if(res != NULL)
        {
            ...
            fprintf(stdout, "path2entry() return : %d %s\n", res->chnid, res->desc);
...
static int open_next(uint8_t chnid)
{
    ...
    for(int i = 0 ; i < channel[chnid].mp3glob.gl_pathc;i++)
    {
        ...
        {
            fprintf(stderr, "open(%s):%s", channel[chnid].mp3glob.gl_pathv[chnid], strerror(errno));
    ...
    fprintf(stderr, "None of mp3 in channel %d id available.", chnid);
    return -1;
}
...
ssize_t mlib_readchn(uint8_t chnid, void *buf, size_t size)
{
    ...
    while(1)
    {
        ...
        else if(len == 0)
        {
            ...
            fprintf(stdout, "media %s file is over", channel[chnid].mp3glob.gl_pathv[channel[chnid].pos]);
            #ifdef DEBUG
        ...
        else/*len > 0*///真正读取到了数据
        {
            ...
            fprintf(stdout, "epoch : %f", (channel[chnid].offset) / (16*1000*1.024));
